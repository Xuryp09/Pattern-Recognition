...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        threshold_matrix = featureVals.copy()
        threshold_matrix = np.where(threshold_matrix < 0, 1, 0)
        error = np.zeros(featureVals.shape[0])

        for i in range(featureVals.shape[0]):
            for j in range(featureVals.shape[1]):
                error[i] += (weights[j] * abs(threshold_matrix[i][j] - labels[j]))
        
        bestIndex = np.argmin(error)
        bestClf = WeakClassifier(features[bestIndex])
        bestError = error[bestIndex]
